...
class GeneradorOraciones:
    ...
    def _generar_frases_normales(self, chunks: Chunks) -> List[FraseGenerada]:
        ...
        if not verbo_completo:
            frase_error = FraseGenerada(
                texto="No se pudo construir oración (falta verbo)"
            )
            return [frase_error]
        ...
        for fn in chunks.fn:
            ...
            if tiene_participio:
                ...
                datos_completos = all(
                    [fn.genero, fn.numero, chunks.verbo_gen, chunks.verbo_num]
                )
    ...
    def _construir_frase(
        self,
        sujeto: Optional[str],
        adv_negacion: Optional[str],
        pron_imp: Optional[str],
        verbo_completo: str,
        objeto: Optional[str],
    ...
    def _generar_frases_ambiguas(self, chunks: Chunks) -> List[FraseGenerada]:
        ...
        for combo in chunks.combinaciones_ambiguas:
            ...
            frase_generada = FraseGenerada(texto=frase, es_ambigua=True)
